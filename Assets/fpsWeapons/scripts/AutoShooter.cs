		...
		if (!myanimation.isPlaying || myanimation.IsPlaying(ambientAnim.name)) 
		{
			if (!isreloading && canfire) {
				if (weaponnextfield == weaponaimFOV)
					...
					inventory.showAIM (true);
				if ((ControlFreak2.CF2Input.GetButton ("Fire1") || ControlFreak2.CF2Input.GetAxis ("Fire1") > 0.1) && currentammo > 0 ) 
				{
				...
				else if (ControlFreak2.CF2Input.GetButton("ThrowGrenade") && inventory.grenade>0)
				{
				...
				else if (ControlFreak2.CF2Input.GetButton("Melee"))
				{
				...
				else if(ControlFreak2.CF2Input.GetButton("Reload") ){
					//Debug.Log("RELOAD");
