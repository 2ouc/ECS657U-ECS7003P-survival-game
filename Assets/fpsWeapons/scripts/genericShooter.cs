...
public class genericShooter : MonoBehaviour {
	...
	public Transform MainBulletPos;
	public Transform ExactBulletPos;
	
	void Awake()
	{
		
		MainBulletPos.transform.position = ExactBulletPos.transform.position;
	...
	void Start()
	{
		...
		GameManager.instance.CanHeadShot = true;
    ...
    void Update () 
	{
		...
		{	
			...
			if (ammo <= 0)
			{
				canfire = false;
				canreload = false;
				if ((ControlFreak2.CF2Input.GetButton("Fire1") || ControlFreak2.CF2Input.GetAxis ("Fire1")>0.1) && !myAudioSource.isPlaying)
				{
