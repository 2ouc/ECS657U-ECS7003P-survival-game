...
public class shotgun : MonoBehaviour {
	...
	public Text Remaining_Ammo;
	void Awake()
	{
		
			//GameManager.instance.CanHeadShot = false;
        
		Debug.Log("Awake HeadShot"+GameManager.instance.CanHeadShot);
    ...
    {
		GameManager.instance.CanHeadShot = false;
		Debug.Log("Onenable False"+ GameManager.instance.CanHeadShot);
	}
	void Start()
	{
	...
	void Update () 
	{
		GameManager.instance.CanHeadShot = false;
		//Debug.Log("Onenable False" + GameManager.instance.CanHeadShot);
		TotalAmmo.text = clipSize.ToString();
		Remaining_Ammo.text = currentammo.ToString();
	...
	//	float newField = Mathf.Lerp(Camera.main.fieldOfView, nextField, Time.deltaTime * 2);
		...
		else
		{
			...
			if (((ControlFreak2.CF2Input.GetButton("Aim") || ControlFreak2.CF2Input.GetAxis("Aim") > 0.1)) && canaim && !playercontrol.running)
			{
		...
		if (currentammo == 0 || currentammo  <= 0 )
		{
			if (ammo <= 0)
			{
				canfire = false;
				canreload = false;
				if ((ControlFreak2.CF2Input.GetButton("Fire1") || ControlFreak2.CF2Input.GetAxis ("Fire1")>0.1) || raycastfire.playerDetected && !myAudioSource.isPlaying)
				{
		...
		if(!isreloading && canfire){
			...
			if ((ControlFreak2.CF2Input.GetButton("Fire1")  || ControlFreak2.CF2Input.GetAxis ("Fire1")>0.1) || raycastfire.playerDetected)
			{
				fire();
			}
		}else{
	...
	IEnumerator setreload()
	{
		...
		while(currentammo != clipSize && ammo >= 1 )
		{
			...
			yield return new WaitForSeconds (myanimation[reloadonceAnim.name].length * 0.4f);
			...
			yield return new WaitForSeconds (myanimation[reloadonceAnim.name].length * 0.6f);
